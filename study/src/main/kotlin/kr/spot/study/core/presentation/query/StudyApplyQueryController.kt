...
class StudyApplyQueryController(
    ...
    fun getMyAppliedStudies(
        @Parameter(hidden = true) @RequestHeader("X-Member-Id") memberId: Long
    ...
    @Deprecated("Use approveStudyApplication instead")
    @Operation(
        summary = "[사용 X] 스터디 신청 내역 조회",
        description = "특정 스터디의 신청 내역을 조회합니다. (스터디장 권한)"
    )
    @ApiResponses(
        io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
            description = "조회 성공",
            content = [Content(schema = Schema(implementation = GetAppliesResponse::class))]
        )
    )
    ...
    fun getStudyApplies(
        ...
        @Parameter(hidden = true) @RequestHeader("X-Member-Id") memberId: Long
    ): ResponseEntity<ApiResponse<GetAppliesResponse>> =
        ResponseEntity.ok(
            ApiResponse.ok(
